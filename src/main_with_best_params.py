#!/usr/bin/env python3
"""
Arquivo principal para experimentação com modelos de classificação
usando os melhores hiperparâmetros já otimizados (20251012).
"""

import sys
import os
import argparse
sys.path.append('/Users/i583975/git/tcc')

import numpy as np
import pandas as pd
from processing import prepare_dataset, get_dataset_info, split_dataset
from process_data import get_canonical_genes, get_candidate_genes
from models import (
    optimize_decision_tree_classifier,
    optimize_random_forest_classifier,
    optimize_gradient_boosting_classifier,
    optimize_hist_gradient_boosting_classifier,
    optimize_knn_classifier,
    optimize_mlp_classifier,
    optimize_svc_classifier,
    optimize_catboost_classifier
)
from reports import summarize_optimized_results
import warnings
warnings.filterwarnings('ignore')

# Melhores hiperparâmetros obtidos em 12/10/2025
BEST_HYPERPARAMETERS = {
    "decision_tree": {
        "max_depth": 3,
        "min_samples_split": 16,
        "min_samples_leaf": 1,
        "criterion": "gini"
    },
    "random_forest": {
        "n_estimators": 200,
        "max_depth": 7,
        "min_samples_split": 11,
        "min_samples_leaf": 3,
        "max_features": "sqrt",
        "criterion": "gini"
    },
    "gradient_boosting": {
        "n_estimators": 150,
        "learning_rate": 0.041901086122355526,
        "max_depth": 4,
        "min_samples_split": 6,
        "min_samples_leaf": 8,
        "subsample": 0.8750118011622728
    },
    "histogram_gradient_boosting": {
        "max_iter": 68,
        "learning_rate": 0.07520558542833868,
        "max_depth": 4,
        "min_samples_leaf": 20,
        "l2_regularization": 0.24633014373667755
    },
    "k_nearest_neighbors": {
        "n_neighbors": 20,
        "weights": "distance",
        "algorithm": "auto",
        "p": 2
    },
    "multi_layer_perceptron": {
        "hidden_layer_sizes": (131, 115, 30),
        "activation": "logistic",
        "alpha": 0.07698392854033051,
        "learning_rate": "constant",
        "max_iter": 742
    },
    "support_vector_classifier": {
        "kernel": "rbf",
        "C": 0.3378198845315039,
        "gamma": "scale"
    },
    "catboost": {
        "iterations": 256,
        "learning_rate": 0.019261294694430553,
        "depth": 7,
        "l2_leaf_reg": 2.9603859610055303,
        "border_count": 239,
        "bagging_temperature": 0.30218907193827815,
        "random_strength": 0.8356422919240496,
        "verbose": False,
        "allow_writing_files": False
    }
}

def get_data_paths(use_renan=False):
    """
    Retorna os caminhos dos arquivos baseado na fonte de dados escolhida
    
    Args:
        use_renan (bool): Se True, usa arquivos do Renan; se False, usa arquivos da Ana
        
    Returns:
        tuple: (features_path, labels_path, data_source)
    """
    if use_renan:
        features_path = "../renan/data_files/omics_features/UNION_features.tsv"
        labels_path = "../renan/data_files/labels/UNION_labels.tsv"
        data_source = "RENAN"
        print(f"📁 Usando dados do RENAN:")
        print(f"   Features: renan/data_files/omics_features/UNION_features.tsv")
        print(f"   Labels: renan/data_files/labels/UNION_labels.tsv")
        print(f"   Formato labels: gene, label (True/False/NaN)")
    else:
        features_path = "./data/UNION_features.tsv"
        labels_path = "./data/processed/UNION_labels.tsv"
        data_source = "ANA"
        print(f"📁 Usando dados da ANA:")
        print(f"   Features: data/UNION_features.tsv")
        print(f"   Labels: data/processed/UNION_labels.tsv") 
        print(f"   Formato labels: genes, 2class (binary), 3class (multiclass)")
    print()
    
    return features_path, labels_path, data_source

def parse_arguments():
    """
    Processa argumentos da linha de comando
    
    Returns:
        argparse.Namespace: Argumentos processados
    """
    parser = argparse.ArgumentParser(
        description='Experimentação com modelos usando melhores hiperparâmetros (20251012)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos de uso:
  python main_with_best_params.py                    # Usa arquivos da Ana, classificação binária (padrão)
  python main_with_best_params.py -renan             # Usa arquivos do Renan (formato original)
  python main_with_best_params.py -multiclass        # Usa classificação multiclasse (TSG vs Oncogene vs Passenger)
  python main_with_best_params.py -multiclass -renan # Combina ambas opções
  python main_with_best_params.py --optimize         # Executa otimização normal ao invés de usar parâmetros fixos
  python main_with_best_params.py --help             # Mostra esta ajuda
        """
    )
    
    parser.add_argument(
        '-renan', '--renan', 
        action='store_true',
        help='Usa arquivos de dados do Renan (formato original: gene, label)'
    )
    
    parser.add_argument(
        '-multiclass', '--multiclass',
        action='store_true',
        help='Usa classificação multiclasse (TSG=1, Oncogene=2) ao invés de binária (cancer=1)'
    )
    
    parser.add_argument(
        '--optimize',
        action='store_true',
        help='Executa otimização normal ao invés de usar hiperparâmetros pré-definidos'
    )
    
    return parser.parse_args()

def run_single_model_with_params(model_name, optimizer_func, X, y, hyperparams, use_optimization=False):
    """
    Executa um único modelo de classificação com hiperparâmetros específicos
    
    Args:
        model_name (str): Nome do modelo
        optimizer_func (function): Função de otimização do modelo
        X (np.array): Features
        y (np.array): Labels
        hyperparams (dict): Hiperparâmetros específicos do modelo
        use_optimization (bool): Se True, executa otimização; se False, usa hiperparâmetros fixos
        
    Returns:
        dict: Resultados do modelo
    """
    print("="*80)
    print(f"EXECUTANDO MODELO: {model_name}")
    if use_optimization:
        print("MODO: Otimização com Optuna")
    else:
        print("MODO: Hiperparâmetros pré-definidos (20251012)")
        print(f"Hiperparâmetros: {hyperparams}")
    print("="*80)
    
    try:
        if use_optimization:
            # Executa otimização normal (com salvamento automático)
            best_model, test_metrics = optimizer_func(X, y, n_trials=30, save_results=True)
        else:
            # Executa com hiperparâmetros fixos
            # Passa os hiperparâmetros como parâmetro adicional
            best_model, test_metrics = optimizer_func(
                X, y, 
                n_trials=1,  # 1 trial apenas
                save_results=True, 
                fixed_params=hyperparams  # Hiperparâmetros fixos
            )
        
        results = {
            'model_name': model_name,
            'status': 'success',
            'model': best_model,
            'test_metrics': test_metrics,
            'hyperparams_used': hyperparams if not use_optimization else 'optimized'
        }
        
        print(f"✓ {model_name} executado com sucesso!")
        return results
        
    except Exception as e:
        print(f"✗ Erro ao executar {model_name}: {e}")
        return {
            'model_name': model_name,
            'status': 'error',
            'error': str(e),
            'model': None,
            'hyperparams_used': hyperparams if not use_optimization else 'failed'
        }

def run_all_models_with_best_params(X, y, use_optimization=False):
    """
    Executa todos os modelos de classificação com os melhores hiperparâmetros
    
    Args:
        X (np.array): Features
        y (np.array): Labels
        use_optimization (bool): Se True, executa otimização; se False, usa hiperparâmetros fixos
        
    Returns:
        list: Lista com resultados de todos os modelos
    """
    # Definição dos modelos e suas funções de otimização
    models_config = [
        ("Decision Tree", optimize_decision_tree_classifier, BEST_HYPERPARAMETERS["decision_tree"]),
        ("Random Forest", optimize_random_forest_classifier, BEST_HYPERPARAMETERS["random_forest"]),
        ("Gradient Boosting", optimize_gradient_boosting_classifier, BEST_HYPERPARAMETERS["gradient_boosting"]),
        ("Histogram Gradient Boosting", optimize_hist_gradient_boosting_classifier, BEST_HYPERPARAMETERS["histogram_gradient_boosting"]),
        ("K-Nearest Neighbors", optimize_knn_classifier, BEST_HYPERPARAMETERS["k_nearest_neighbors"]),
        ("Multi-Layer Perceptron", optimize_mlp_classifier, BEST_HYPERPARAMETERS["multi_layer_perceptron"]),
        ("Support Vector Classifier", optimize_svc_classifier, BEST_HYPERPARAMETERS["support_vector_classifier"]),
        ("CatBoost", optimize_catboost_classifier, BEST_HYPERPARAMETERS["catboost"])
    ]
    
    results = []
    
    if use_optimization:
        print("INICIANDO EXPERIMENTAÇÃO COM OTIMIZAÇÃO COMPLETA")
    else:
        print("INICIANDO EXPERIMENTAÇÃO COM HIPERPARÂMETROS PRÉ-DEFINIDOS")
        print("(Melhores hiperparâmetros obtidos em 12/10/2025)")
    
    print(f"Dataset: {X.shape[0]} amostras x {X.shape[1]} features")
    print()
    
    for i, (model_name, optimizer_func, hyperparams) in enumerate(models_config, 1):
        print(f"Progresso: {i}/{len(models_config)} modelos")
        
        result = run_single_model_with_params(
            model_name, optimizer_func, X, y, hyperparams, use_optimization
        )
        results.append(result)
        
        # Breve pausa entre modelos
        import time
        time.sleep(2)
    
    return results

def main(use_renan=False, use_multiclass=False, use_optimization=False):
    """
    Função principal do experimento
    
    Args:
        use_renan (bool): Se True, usa arquivos do Renan; se False, usa arquivos da Ana
        use_multiclass (bool): Se True, usa classificação multiclasse; se False, usa binária
        use_optimization (bool): Se True, executa otimização; se False, usa hiperparâmetros fixos
    """
    print("CLASSIFICAÇÃO DE GENES-ALVO USANDO HIPERPARÂMETROS OTIMIZADOS")
    print("="*80)
    
    # Obtém os caminhos dos arquivos baseado na fonte escolhida
    features_path, labels_path, data_source = get_data_paths(use_renan)
    
    # Verifica se os arquivos existem
    if not os.path.exists(features_path):
        print(f"❌ Arquivo de features não encontrado: {features_path}")
        return
    
    if not os.path.exists(labels_path):
        print(f"❌ Arquivo de labels não encontrado: {labels_path}")
        return
    
    print("✅ Arquivos encontrados com sucesso!")
    
    # Prepara o dataset
    print("🔄 Carregando e preparando dados...")
    classification_type = 'multiclass' if use_multiclass else 'binary'
    X, y, gene_names, feature_names = prepare_dataset(features_path, labels_path, classification_type)
    
    if X is None:
        print("❌ Erro ao preparar dataset. Abortando.")
        return
    
    # Mostra informações do dataset
    dataset_info = get_dataset_info(X, y, gene_names, feature_names)
    print("\n📊 INFORMAÇÕES DO DATASET:")
    print(f"  Fonte de dados: {data_source}")
    print(f"  Amostras: {dataset_info['n_samples']}")
    print(f"  Features: {dataset_info['n_features']}")
    print(f"  Distribuição das classes: {dataset_info['class_distribution']}")
    print(f"  Estatísticas das features:")
    print(f"    - Média: {dataset_info['feature_stats']['mean']:.4f}")
    print(f"    - Desvio padrão: {dataset_info['feature_stats']['std']:.4f}")
    print(f"    - Valores zero: {dataset_info['feature_stats']['zeros_percentage']:.2f}%")
    
    # Configuração do experimento
    print(f"\n⚙️  CONFIGURAÇÃO DO EXPERIMENTO:")
    if use_optimization:
        print(f"  Modo: Otimização completa (30 trials por modelo)")
    else:
        print(f"  Modo: Hiperparâmetros pré-definidos (melhores de 12/10/2025)")
    print(f"  Validação: Estratificada 5-fold + Holdout 80/20")
    print(f"  Métrica de otimização: PR AUC (Average Precision)")
    print(f"  Resultados salvos em: /Users/i583975/git/tcc/results/")
    print()
    
    # Executa todos os modelos
    print("🚀 Iniciando experimentos...")
    results = run_all_models_with_best_params(X, y, use_optimization)
    
    # Conta sucessos e falhas
    successful_results = [r for r in results if r['status'] == 'success']
    failed_results = [r for r in results if r['status'] == 'error']
    
    print("\n" + "="*80)
    print("RESUMO DA EXPERIMENTAÇÃO")
    print("="*80)
    print(f"✅ Modelos executados com sucesso: {len(successful_results)}")
    print(f"❌ Modelos com erro: {len(failed_results)}")
    
    if failed_results:
        print("\nErros encontrados:")
        for result in failed_results:
            print(f"  - {result['model_name']}: {result['error']}")
    
    if successful_results:
        print("\n" + "="*80)
        print(f"{'Modelo':<25} {'Accuracy':<10} {'Precision':<11} {'Recall':<8} {'F1':<8} {'ROC AUC':<9} {'PR AUC':<8}")
        print("="*80)
        
        for result in successful_results:
            metrics = result['test_metrics']
            model_name = result['model_name']
            print(f"{model_name:<25} "
                  f"{metrics.get('accuracy', 0):<10.4f} "
                  f"{metrics.get('precision', 0):<11.4f} "
                  f"{metrics.get('recall', 0):<8.4f} "
                  f"{metrics.get('f1', 0):<8.4f} "
                  f"{metrics.get('roc_auc', 0):<9.4f} "
                  f"{metrics.get('pr_auc', 0):<8.4f}")
        
        print("="*80)
    
    # Gera resumo se houver resultados bem-sucedidos
    if successful_results:
        print("\n📊 Gerando resumo comparativo...")
        try:
            summary_path = summarize_optimized_results()
            if summary_path:
                print(f"✅ Resumo salvo em: {summary_path}")
            else:
                print("⚠️  Não foi possível gerar o resumo")
        except Exception as e:
            print(f"❌ Erro ao gerar resumo: {e}")
    
    print("\n🎉 Experimentação concluída!")
    print("="*80)


if __name__ == "__main__":
    args = parse_arguments()
    main(
        use_renan=args.renan,
        use_multiclass=args.multiclass,
        use_optimization=args.optimize
    )
